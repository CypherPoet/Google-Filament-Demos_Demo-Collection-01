...
- **`<EnvironmentalLight>` double‑release crash.** The built‑in component manually frees its KTX buffer after the first frame; on this stack the effect re‑fires and hits an already‑freed pointer. [`src/components/lighting.tsx`](mobile/src/components/lighting.tsx) provides a drop‑in IBL that never manually releases (it lets the buffer free on unmount).
